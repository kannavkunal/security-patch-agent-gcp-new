...
def validate_message(data: dict) -> Tuple[bool, str]:
    ...
    # Validate repo_url against whitelist (dynamically fetched from GitHub API)
    allowed_repos = get_allowed_repositories()
    if repo_url not in allowed_repos:
        return False, f"Repository not in whitelist: {repo_url}. Allowed: {', '.join(allowed_repos) if allowed_repos else 'None'}"
